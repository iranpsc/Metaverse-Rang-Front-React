...
export default function TicketForm({ setTickets }) {
  ...
  const onSubmit = () => {
    ...
    if(formData.department === 'none') {
      if(!currentUser) {
        errors.push('لطفا فردی را که میخواهید به آن پیامی ارسال کنید وارد کنید و یا موضوع پیام خود را وارد کنید.')
      }
    }
    ...
    if(files.length > 0) {
      ...
      if(currentUser) {
        if(!errors[0]) {
          Request('tickets', HTTP_METHOD.POST, {title: formData.title, content: formData.content, reciever: Number(currentUserId), attachment: attachment}, {"Content-Type": "multipart/form-data"}).then((response) => {
        ...
        if(!errors[0]) {
          Request('tickets', HTTP_METHOD.POST, {...formData, attachment: attachment}, {"Content-Type": "multipart/form-data"}).then((response) => {
            setTickets((prevState) => [...prevState, response.data.data]);
            setFormData(initialState);
            setFiles([]);
            setCurrentUser(null);
      ...
      if(currentUser) {
        if(!errors[0]) {
          Request('tickets', HTTP_METHOD.POST, {title: formData.title, content: formData.content, reciever: Number(currentUserId)}).then((response) => {
            setTickets((prevState) => [...prevState, response.data.data]);
            setFormData(initialState);
            setCurrentUser(null);
          }).catch(error => {
        ...
        if(!errors[0]) {
          Request('tickets', HTTP_METHOD.POST, formData).then((response) => {
            setTickets((prevState) => [...prevState, response.data.data]);
            setFormData(initialState);
            setCurrentUser(null);
          }).catch(error => {
