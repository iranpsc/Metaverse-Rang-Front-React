...
import { SanitizeHTML, TextShorter, ToastError } from '../../../../../Services/Utility';
...
function ChatSection({ ticket, setTicket }) {
  ...
  const onSubmit = () => {
    Request(`tickets/response/${ticket.id}`, HTTP_METHOD.POST, formData).then(response => {
  ...
  const onCloseTicket = () => {
    Request(`tickets/close/${ticket.id}`).then(() => {
  ...
  return (
    <Container>
      <ButtonsContainer>
        <img className='cursor-pointer' src={BackArrowImage} alt='' width={32} onClick={() => {
          setTicket(null);
          setFormData({
            response: ""
          });
      ...
      <Chat ref={ScrollRef} style={{ height: `${parseInt(ticket.status) === 5 && '100%'}` }}>
        <Description>
          <h3>{ticket.sender}</h3>
          <p className='mb-3'><span>{SanitizeHTML(ticket.title)}</span>, {SanitizeHTML(ticket.content)}</p>
          {ticket.attachment !== "https://dl.qzparadise.ir/" && <a href={ticket.attachment} className='link'>فایل ضمیمه</a>}
        </Description>
        ...
        <Table>
          ...
          {tickets.map((report) => (
            <Row key={shortid.generate()}>
              ...
              <Cell>
                <img className='cursor-pointer' src={SeenIcon} alt="" width={32} onClick={() => onClickHandler(report.id)}/>
              </Cell>
            </Row>
          ))}
        </Table>
