...
function NoteTable({ Notes, setNotes }) {
  ...
  return (
    note ? <>
      <img className='cursor-pointer' src={BackArrowImage} alt='' width={32} onClick={() => setNote(null)}/>

      <Container>
        <Description>
          <h3>{note.title}</h3>
          <p className='mb-3'>{SanitizeHTML(note.content)}</p>
          {note.attachment !== "https://dl.qzparadise.ir/" && <a href={note.attachment} className='link'>فایل ضمیمه</a>}
        </Description>
      </Container>
    </> :
    <>
        ...
        {Notes.reverse().map((notes) => (
          <Row key={shortid.generate()}>
            ...
            <Cell style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
              ...
              <img className='cursor-pointer' src={SeenIcon} alt="" width={32} onClick={() => onClickHandler(notes.id)}/>
